...
class SegmentTree:
    ...
    def get_max(self,key,left,right,lower_bound,upper_bound):
        ...
        mid = (left + right) // 2
        max_from_left_tree = self.get_max(key*2+1,left,mid,lower_bound,upper_bound)
        max_from_right_tree = self.get_max(key*2+2,mid+1,right,lower_bound,upper_bound)
...
if __name__=='__main__':
    ...
    for query in range(num_of_queries):
        ...
        print(f'Query {query}: Max element in range [{lower_bound} - {upper_bound}] is {answer}')